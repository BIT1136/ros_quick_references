...
**必须**运行一个roscore才能使ROS节点进行通信。一般先在一个终端启动roscore，然后打开其他终端使用rosrun运行其他程序。

```
roscore [options]

  -p PORT               主节点端口 Only valid if master is launched
  -v                    打印详细信息
  -w NUM_WORKERS
...
使用相应的[[launch文件]]启动多个ROS节点，并自动运行roscore。
...
rostopic pub [options] </topic> <type> [args...]
  ...
  -v                    打印详细信息
  -r RATE               发布消息的频率（hz）。对于来自-f和stdin的输入，默认为
                        10，其他情况未设置。
  -1                    发布一次消息并退出
  -f FILE               从YAML文件读取参数(Bagy)
  -l, --latch           enable latching for -f, -r and piped input.  This latches the first message.
...
用于录制和回放ROS话题的工具，录制结果被称为包，保存为.bag文件。可以使用[[图形界面工具#rqt_bag|rqt_bag]]查看。
